...
class SourceFile:
    ...
    def emit_llvm_bc(self):
        ...
        # print str(self.arguments.clang) + " -emit-llvm -c -g " + str(self.source_file) + " -o " + str(bc_file) + " " + para
    ...
    def emit_llvm_ll_and_functions(self, bitcode_file):
        ...
        zyt-2021.4.21
        主要功能：
            将 .bc 文件转为 .ll 文件，并从中读取函数相关的信息，保存到 functions.txt 中
        """
        ## 1. 创建一个bitcode_file.c目录，将bitcode_file文件拷贝到该目录下，并在该目录下创建两个
        ## 文件：bitcode_file.ll，functions.txt,llvm-dis.log.txt
        ...
        ## self.arguments.llvm_config 保存llvm-dis-3.8所在目录
        llvm_dis = join_path(self.arguments.llvm_config, 'llvm-dis-3.8')
        # llvm_dis = join_path(self.arguments.llvm_config, 'llvm-dis-6.0')
        ## [*]llvm_dis:  /usr/bin/llvm-dis-3.8
        print "[*]llvm_dis: ", llvm_dis

        ## 2. 调用llvm-dis-3.8将 .bc 文件转成 .ll 文件
        LOG = open(llvm_dis_log_file, 'w')
        try:
            if is_file(new_bitcode_file):
                ...
                print '[*]emit_llvm_ll_and_functions, No bc file:', new_bitcode_file
        except:
            print llvm_dis, ' cannot be found'
            return True

        ## 3. 打开.ll文件，读取每一行，若开头是define，那么是一个函数，将其保存到 functions.txt中  
        functions = open(function_names_file, 'w')
        lines = read_lines(ll_file)
        functions_name = []
        prev_line = '' ## 实现追加写
