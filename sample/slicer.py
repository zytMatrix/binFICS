...
class Slicer:
    ...
    def run(self):
        ...
        try:
            ...
            print '[*] Run ...', count, " ", self.pdg_graph_file
            # print "cfg: ", self.arguments.has_control_flow
            
            self.start_time = default_timer()
            if self.arguments.has_control_flow:
                self.find_control_flow_to_graph()

            self.remove_forward_control_dependency() ## 删除蓝色（控制流）和绿色（内存访问）的边
            if self.arguments.has_control_flow:
