...
class BitCodeFile:
    ...
    def extract_pdg(self):
        ...
        ## added by zyt 2021.5.16 
        # print "\n[*] --------------- get functions: ---------------"
        # print self.get_functions()
        # print "[*] -----------------------\n"
        # target_funs = ['dtls1_process_heartbeat', 'tls1_process_heartbeat']
        # for item in self.get_functions():
        #     if item not in target_funs:
        #         print "[*] func_name: ", item
        #         return
        for function_name in self.get_functions():
            ## added by zyt 2021.5.16 
            # print functions_name
            # if functions_name not in target_funs:
            #     continue
    ...
    def get_functions(self):
        ...
        # print read_lines(functions_file)
        try:
            for line in read_lines(functions_file):
                ...
                ## changed by zyt 2021.4.22
                if ' inlinehint ' not in line:
    ...
    def extract_as(self):
        ...
        # print "[*] slice file: ",self.file_info
        if not slicer.error:
            slicer.run()
            del slicer
            return True
    ...
    def extract_potential_features(self):
        """
        NN 类型下提取的特征：
        [*] label:  br i1 label label
        [*] bb_id:  0x1a49200

        [*] label:  add i64
        [*] bb_id:  0x19d66b0

        [*] label:  icmp eq i1 false
        [*] bb_id:  0x1a50b70
        """
    ...
    def extract_node_names_features(self):
        for node in self.afs_graph.nodes:
            ...
            # print "[*] label: ", self.afs_graph.nodes[node]['label']
