...
def process_sample(source_file, args, project_dir, compile_arguments, type=''):
    ...
    # [*] -------------- process_sample ------------
    # [*] args:  <arguments.Arguments instance at 0x7f845c4a34b0>
    # [*] project_dir:  /root/FICS/data/bcs/pdf2svg
    # [*] compile_arguments:  {}
    # [*] type:  pdg
    # [*] -------------- process_sample ------------
    ...
    # print "[*] compile_arguments: ", compile_arguments
    # print "[*] type: ", type
...
class ProjectCode:
    ...
    def retrieve_bc(self):
        ...
        compile_commands_path = join_path(self.project_dir, 'compile_commands.json')
        print "[*] compile_commands_path: ", compile_commands_path
        # exit(1)
        # compile_commands = []
    ...
    def prepare_bc(self):
        ## [*] bc_dir:  /root/FICS/data/bcs/libtiff-19f6b70
        bc_dir = self.get_bc_dir_of_project() 
        # print "[*] bc_dir: ",bc_dir
        # print self.arguments.search_spaces ## 默认为空
    ...
    def retrieve_pdg(self):
        ## bcs_dir: /root/FICS/data/bcs/pdf2svg
        bcs_dir = self.get_bc_dir_of_project()

        ## bcs_files：/root/FICS/data/bcs/pdf2svg/pdf2svg-o1.c/pdf2svg-o1.bc
        bcs_files = get_files_in_dir(bcs_dir, ext='.bc')

        if len(bcs_files) == 0:
            print 'No bitcode found'
            return
        ProjectCode.pbar = tqdm(total=len(bcs_files)) ## tqdm：一个快速,可扩展的Python进度条
        p = Pool(processes=self.arguments.count_cpu)
    ...
    def retrieve_as(self):
        ...
        p_main = Pool(processes=self.arguments.count_cpu - 2)
        # global p_main
        for pdg_file in pdg_files:
            p_main.apply_async(process_sample, (pdg_file, self.arguments, bcs_dir, {}, 'as'),
                          callback=my_callback)
    ...
    def extract_features(self, save=False):
        ## self.feature_type：['afs', 'NN']
        ## 最终调用 extract_features_afs
        print(self.feature_type[0])
        print(self.feature_type[1])
    ...
    def extract_features_afs(self, save=False):
        ...
        ## afs_files: 全部的以_ODD开头的文件，变量的DDG
...
## added by zyt 2021.5.17

# if __name__ == "__main__":
#     global p_main
#     print "[*] ", self.arguments.count_cpu
#     p_main = Pool(processes=self.arguments.count_cpu)
